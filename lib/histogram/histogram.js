 ...
 * @param  {Array.<number>} data in the format [x,y,...]
 * @param  {number} optOrMethod Optional settings or `method`.
 * @param  {Object|string} optOrMethod.method 'squareRoot' | 'scott' | 'freedmanDiaconis' | 'sturges'
 * @param  {Array.<number>|number} optOrMethod.dimensions If data is a 2-d array,
 *         which dimension will be used to calculate histogram.
 ...
 * @return {Object}
 */
export function computeBins2dHeatmap(dataXY, optOrMethod, numberOfBinsX, numberOfBinsY ) {
    var arrayColumn = (arr, n) => arr.map(x => x[n]);
    var dataX = arrayColumn(dataXY, 0);
    var dataY = arrayColumn(dataXY, 1);
    ...
    for (var i = 0; i < data_x.bins.length; i++) {
        ...
        filtered_arr.forEach((innerArray) => {
            var y_bin = Math.floor((innerArray[1] - ymin) / stepY);
...
/**
 * Generate data in ECharts heatmap format for 2D histograms: [bin_x,bin_y,average value]
 * @param  {Array.<number>} data in the format [x,y,...,...,value] where x,y,value are at index 0,1,4 respectively
 * @param  {number} optOrMethod Optional settings or `method`.
 * @param  {Object|string} optOrMethod.method 'squareRoot' | 'scott' | 'freedmanDiaconis' | 'sturges'
 * @param  {Array.<number>|number} optOrMethod.dimensions If data is a 2-d array,
 *         which dimension will be used to calculate histogram.
 ...
 * @return {Object}
 */
export function computeBins2dHeatmapAverage(dataXYZ, optOrMethod, numberOfBinsX, numberOfBinsY ) {
    var arrayColumn = (arr, n) => arr.map(x => x[n]);
    var dataX = arrayColumn(dataXYZ, 0);
    var dataY = arrayColumn(dataXYZ, 1);
    var dataZ = arrayColumn(dataXYZ, 4);

    if(dataX.length !== dataY.length || dataX.length !== dataZ.length || dataX.length == 0){
        console.log('computeBins2dHeatmap: dataX.length must be equal to dataY.length and dataZ.length and > 0 to compute!');
        return {
            data: [],
            x_axis: [],
            y_axis: []
    ...
    for (var i = 0; i < data_x.bins.length; i++) {
        ...
        var filtered_arr = dataXYZ.filter(innerArray => (innerArray[0] < xmax && innerArray[0] >= xmin));

        filtered_arr.forEach((innerArray) => {
            var y_bin = Math.floor((innerArray[1] - ymin) / stepY);
            y_bins[y_bin] = (y_bins[y_bin] + 1) || 1;
            sum[y_bin] = (sum[y_bin]+innerArray[4]) || innerArray[4];
        });

        for (const [y_bin, count] of Object.entries(y_bins)) {
            sum[y_bin] = (sum[y_bin] / count).toFixed(2);
            data.push([i, parseInt(y_bin), sum[y_bin]]);
        }
    }
